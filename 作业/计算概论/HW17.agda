...
module BMF2-1 where
  ...
  split {A} = reduce f ∘ map g
    where
      g : A → List A × A
      g x = ( [] , x )
      f : List A × A → List A × A → List A × A
      f (xs , x) (ys , y) = (x ∷ ys , y)
  ...
  reduce-map-is-homomorphism f _⊗_ prop .distrib ([_]′ x) ys = refl
  reduce-map-is-homomorphism f _⊗_ prop .distrib (x ∷′ xs) ys
    rewrite reduce-map-is-homomorphism f _⊗_ prop .distrib xs ys = sym (⊗-assoc (f x) (reduce _⊗_ (map f xs)) (reduce _⊗_ (map f ys)))
      where
        open IsSemigroup prop renaming (assoc to ⊗-assoc)
  ...
  split-is-indeed-correct (x ∷′ xs) rewrite split-is-indeed-correct xs = refl
  ...
  init-is-not-homomorphism {_⊗_} m H = ¬K K 
    where
      ...
      K = 
        ...
        ≡⟨⟩
          init ([_]′ 2) ⊗ init ([_]′ 2)
        ≡⟨ sym ( distribH ([_]′ 2) ([_]′ 2) )⟩
          init ( 2 ∷′ ([_]′ 2) )
        ≡⟨⟩
          [ 2 ]
        ∎
        where
          open IsHomomorphism H renaming (distrib to distribH)
      ¬K : [ 1 ] ≢ [ 2 ]
      ¬K ()
